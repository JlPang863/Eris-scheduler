...
class Antman_Scheduler(object):
    def __init__(self, name, logger, scheduler_queue, hub_queue, timer):
        ...
        self.logger.debug(self.name + "152:: " + "self.cluster_num_cpu: " + str(self.cluster_num_cpu))
    ...
    def _recompute_priority(self, time_slot):
        min_priority = float('inf')
        for job in self.uncompleted_job_arr.keys():
            job.j_priority = job.accum_running_time * job.allocated_workers - time_slot
            if job.j_priority < min_priority:
                min_priority = job.j_priority

            self.uncompleted_job_arr[job] = job.j_priority
    ...
    def _schedule(self, time_slot):
        ...
        self._recompute_priority(time_slot)
        for job in new_jobs:
