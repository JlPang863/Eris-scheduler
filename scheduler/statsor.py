    ...
    def _msg_handle(self):
        while not self.exit_flag:
            try:
                ...
                continue
            self.logger.debug(self.name + " 35:: " + str((t, src, dest, type, job)))
        ...
        self.logger.debug(self.name + " 45:: " + self.name + " has exited.")

    def _stats(self, t):
        self.logger.info(self.name + " 48:: " + "time slot: " + str(t) + "")
        num_submit_jobs = len(self.scheduler.uncompleted_jobs) + len(self.scheduler.completed_jobs) + len(self.scheduler.rejected_jobs)
        num_completed_jobs = len(self.scheduler.completed_jobs)
        num_uncompleted_jobs = len(self.scheduler.uncompleted_jobs)
        num_rejected_jobs = len(self.scheduler.rejected_jobs)
        self.logger.info(self.name + ":: " + "submitted jobs: " + str(num_submit_jobs) + ", " +
                         ...
                         "rejected_jobs: " + str(num_rejected_jobs))

        self.logger.info(self.name + ":: sum(self.scheduler.node_used_cpu_list[t])" + str(self.scheduler.node_used_cpu_list[t]))
        
        cluster_cpu_util = float('%.3f' % (1.0 * sum(self.scheduler.node_used_cpu_list[t]) / self.scheduler.cluster_num_cpu))
        cluster_mem_util = float('%.3f' % (1.0 * sum(self.scheduler.node_used_mem_list[t]) / self.scheduler.cluster_num_mem))
        cluster_gpu_util = float('%.3f' % (1.0 * sum(self.scheduler.node_used_gpu_list[t]) / self.scheduler.cluster_num_gpu))
        ...
        try:
            ...
            self.logger.debug(self.name + " 79:: " + "No jobs are finished!!!")
        else:
            self.logger.debug(
        ...
        if self.scheduler.name == "FIFO_Scheduler" or self.scheduler.name == "DPS_Scheduler" or self.scheduler.name == "DRF_Scheduler":
