...
class Estimator(object):
    ...
    def test_speed(self, new_jobs):
        ...
        while True:
            ...
            collected = True
            for job in new_jobs:
                if len(job.training_speeds) < 1: # collect 2 points
                    collected = False
                    break

            if collected:
                # all job has collected 2 points
                self.logger.info("No need to test speed, all jobs are known workload.")
                break
            else:
                # at least one job does not have 2 speed points
                for job in new_jobs:
            ...
            while not q.empty():
                ...
                while True:
                    ...
                    if (job.num_ps, job.num_worker) in job.training_speeds: # avoid repetition
                        ...
                        self.logger.debug(self.name + ":: " + job.name + " infinite loop")
                        continue
                    else:
            ...
            for job in sorted_jobs:
                ...
                if len(placement) == job.num_ps: 
                    ...
                    self.logger.debug(self.name + ":: " + job.name + " does not get resources to test speed")  #wait to get resources
            ...
            for job in running_jobs:
                flag = True
                while flag:
                    speed_list = job.get_training_speed() # Speed: 5.21 samples/sec 
                    if min(speed_list) > 0:
            ...
            for job in new_jobs:
                self.logger.debug(self.name + " 185 :: " + job.name + ": " + str(job.training_speeds))
