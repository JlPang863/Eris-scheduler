...
class FIFO_Scheduler(object):
    ...
    def _schedule(self,time_slot):
        ...
        for job in new_jobs:
            ...
            worker_upper_bound = 0
            for r in range(cluster_num_nodes):
                if job.worker_gpu == 0:
                    worker_upper_bound += int(min((params.CPU_PER_NODE - self.node_used_cpu_list[time_slot][r]) // job.worker_cpu,
                                   (params.MEM_PER_NODE - self.node_used_mem_list[time_slot][r]) // job.worker_mem))
                else:
                    worker_upper_bound += int(min((params.CPU_PER_NODE - self.node_used_cpu_list[time_slot][r]) // job.worker_cpu,
                                   (params.MEM_PER_NODE - self.node_used_mem_list[time_slot][r]) // job.worker_mem,
                                   (params.GPU_PER_NODE - self.node_used_gpu_list[time_slot][r]) // job.worker_gpu))
            ...
            worker_num=random.randint(2, math.floor(min(worker_upper_bound, int(math.ceil(float(job.data_chunks * job.num_epochs)/job.P)))))
            self.logger.debug(self.name + "152:: " + "worker_upper_bound: " + str(worker_upper_bound))
            self.logger.debug(self.name + "152:: " + "math.ceil(job.data_chunks/job.P: " + str(math.ceil(float(job.data_chunks * job.num_epochs)/job.P)))
        ...
        for job in self.uncompleted_jobs:
            ...
            self.logger.debug(self.name + ":: scheduling results" + " num_ps: " + str(job.num_ps) + " num_worker: " + \
                str(job.num_worker))
            self.logger.debug(self.name + " 225:: " + "job.worker_placement_t: " + str(job.num_worker_t))
            self.logger.debug(self.name + " 225:: " + "job.ps_placement_t: " + str(job.num_ps_t))
