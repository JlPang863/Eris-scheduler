...
def update_progress(logfile, recordfile):
    ...
    while True:
            ...
            for line in f:
                ...
                if epoch_index > -1:
                    ...
                    if batch_index > -1:
                        ...
                        #batch = str(line[line.find("[", batch_index,batch_index+10) + 1: line.find("]", batch_index)]) #batch = 400-410
        ...
        stat_dict['train-loss'] = train_loss   # only need the last one
        stat_dict['val-acc'] = val_acc
        stat_dict['val-loss'] = val_loss       # needed at scheduler/progressor.py
        stat_dict['time-cost'] = time_cost
        sum_cost = 0
        if len(time_cost) > 0:
...
def main():
    # new add
    cmd  = 'touch ' + WORK_DIR + 'training.log'
    os.system(cmd)
