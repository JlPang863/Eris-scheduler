...
def load_mnist_data(path, kind): 
    ...
    with gzip.open(labels_path, 'rb') as lbpath:
        ...
        label = np.fromstring(lbpath.read(),dtype=np.uint8)
    with gzip.open(images_path, 'rb') as imgpath:
        magic, num, rows, cols = struct.unpack('>IIII',imgpath.read(16))
        image = np.fromstring(imgpath.read(),dtype=np.uint8).reshape(len(label), rows, cols)
    return (label, image)
...
def get_mnist_iter(args, kv):
    ...
    (train_lbl, train_img) = load_mnist_data(
            os.path.join('data'), kind='train')
    (val_lbl, val_img) = load_mnist_data(
            os.path.join('data'), kind='t10k')
