...
def _create_kvstore(kvstore, num_device, arg_params):
    ...
    if kvstore is None:
        ...
        if num_device is 1 and 'dist' not in kvstore:
            ...
            print 'the max_size of parameters transform between workers and ps:' + str(max_size)
            if kvstore is 'local':
                # automatically select a proper local, what is the meaning?
                max_size = max(np.prod(param.shape) for param in
                               arg_params.values())
                print 'DIR: /mxnet/python/'
                print 'the max_size of parameters transform between workers and ps:' + str(max_size)
...
def _train_multi_device(symbol, ctx, arg_names, param_names, aux_names,
    ...
    if logger is None:
        logger = logging
    executor_manager = DataParallelExecutorManager(symbol=symbol,    #data parallel
                                                   sym_gen=sym_gen,
                                                   ctx=ctx,
    ...
    for epoch in range(begin_epoch, end_epoch):
        ...
        while True:
            ...
            for data_batch in train_data:
                ...
                executor_manager.forward(is_train=True)  #forward algorithm 
                executor_manager.backward()   # backward algorithm
